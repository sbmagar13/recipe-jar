...
test('an already-metric recipe shows no dead toggle', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: /type in one of your own/i }).click()
  await page.getByLabel('Recipe name').fill('Metric Native')
  await page.getByLabel(/Ingredients/).fill('400 g noodles\n2 tsp soy sauce\n1 tbsp oil')
